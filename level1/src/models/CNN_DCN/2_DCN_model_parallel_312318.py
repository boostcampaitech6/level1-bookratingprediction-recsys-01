...
class DeepCrossNetworkModel(nn.Module):
    def __init__(self, args, data):
        ...
        self.embed_output_dim = len(self.field_dims) * args.embed_dim + 18 # 전체 embedding된 x의 dimension + 18
        self.cn = CrossNetwork(self.embed_output_dim - 18, args.num_layers)
        ...
        self.cd_linear_parallel1 = nn.Linear(self.embed_output_dim-18 + args.mlp_dims[-1], (self.embed_output_dim-18 + args.mlp_dims[-1])//3, bias=True)
        self.cd_linear_parallel2 = nn.Linear((self.embed_output_dim-18 + args.mlp_dims[-1])//3, 1, bias=True)
        self.cd_linear_parallel = nn.Linear(self.embed_output_dim-18 + args.mlp_dims[-1], 1, bias=True)

        self.bn = nn.BatchNorm1d((self.embed_output_dim-18 + args.mlp_dims[-1])//3)
    ...
    def forward(self,x: torch.Tensor):
        ...
        # p2 = self.cd_linear_parallel2(p1)
    ...
    #     x_l1 = self.cn(embed_x) # cross network
    #     x_out = self.mlp(x_l1) # MLP
    #     p = self.cd_linear(x_out) # output layer
    #     return p.squeeze(1)
