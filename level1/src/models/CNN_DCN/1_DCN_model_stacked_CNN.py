...
class DeepCrossNetworkModel(nn.Module):
    def __init__(self, args, data):
        ...
        self.embed_output_dim = len(self.field_dims) * args.embed_dim + 12 # 전체 embedding된 x의 dimension + 18
        ...
        self.cd_linear_stacked = nn.Linear(args.mlp_dims[-1], 1, bias=False)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 2) CONTEXT
        embed_x = self.embedding(user_isbn_vector).view(-1, self.embed_output_dim-12)
        x_l1 = self.cn(embed_x) # cross network

        # 3) CONCAT and MLP
        x_l2 = torch.concat([x_l1, img_feature], dim=1)
        x_out = self.mlp_stacked(x_l2) # MLP
        p = self.cd_linear_stacked(x_out) # output layer
        return p.squeeze(1)