...
class FactorizationMachine(nn.Module):
    ...
    def forward(self, x):
        linear = self.linear(x) #FM:FeaturesLinear
...
class FeaturesEmbedding(nn.Module):
    def __init__(self, field_dims: np.ndarray, embed_dim: int):
        ...
        self.offsets = np.array((0, *np.cumsum(field_dims)[:-1]), dtype=np.int32) # field dims를 고치기
...
class CNN_FM(torch.nn.Module):
    def __init__(self, args, data):
        ...
        self.cnn = CNN_Base()
        # self.cnn = torch.load_state_dict(torch.load('saved_models/1x1_CNN_FM_baseline.pt'))
        # for param in self.cnn.parameters():
        #     param.requires_grad=False

        self.fm = FactorizationMachine(
                                        input_dim=(args.cnn_embed_dim * len(data['field_dims'])) + (12 * 1 * 1),
    ...
    def forward(self, x):
        ...
        img_feature = self.cnn(img_vector) # image embedding
