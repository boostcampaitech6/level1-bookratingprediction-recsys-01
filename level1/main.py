...
from src.train import train, train1, train2, test
...
def main(args):
    ...
    if args.model in ('FM', 'FFM', 'WDN'):
        data = context_data_load(args)
    elif args.model in ('NCF', 'DCN'):
        data = integrated_data_load(args)
    ...
    elif args.model in ('CNN_FM','CNN_context_FM'):
        data = integrated_data_load(args)
    else:
        pass
    ...
    if args.model in ('FM', 'FFM','WDN'):
        ...
        #data = integrated_data_split(args, data)
        data = integrated_data_loader(args, data)
    ...
    model = train1(args, model, data, logger, setting)
...
if __name__ == "__main__":
    ...
    arg('--model', type=str, choices=['FM', 'FFM', 'NCF', 'WDN', 'DCN', 'CNN_FM', 'DeepCoNN','CNN_context_FM'],
